...
aggregate_output_immi <- function(deSolveOut, times, nAges, nState, nRet, old = 0)
{
  ...
  listNames <- c("deSolve", "unProt_first", "unProt_second", "unProt_combined",
                 "mAb_first", "mAb_second", "mAb_combined",
                        ...
                        text = "")
{
  if(!is.na(obs3)[1])
  {
    ...
    colnames(plotData) <- c("Month", "obs1", "obs2", "obs3")

    g1 <- ggplot2::ggplot(as.data.frame(plotData), ggplot2::aes(x = Month)) +
      ggplot2::geom_line(ggplot2::aes(y = obs1), size = 1.1) +
      ggplot2::geom_line(ggplot2::aes(y = obs2), color="steelblue", linetype="twodash", size = 1.1) +
      ggplot2::geom_line(ggplot2::aes(y = obs3), color = "darkred", linetype = "dashed", size = 1.1) +
      ggplot2::scale_x_continuous(name = "Month", breaks = 1:nrow(plotData), labels = plotData[,1])+
      ggplot2::ylab(ylabel)+
      ggplot2::theme_bw()+
      ggplot2::ggtitle(text)+
      ggplot2::theme(plot.title = ggplot2::element_text(size = 10))
